...
function EmotionRankDetail() {
  return (
    <DetailComponent>
      <DetailItems>{categoryEmotions1.map(item => <EmotionRankItem key={item.categoryName} info={item} />)}</DetailItems>
      <DetailItems>{categoryEmotions2.map(item => <EmotionRankItem key={item.categoryName} info={item} />)}</DetailItems>
    </DetailComponent>
  )
};
