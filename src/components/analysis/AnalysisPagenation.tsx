...
const ArrowLeftImage = styled.div<{ $isEnd: string }>`
  background-image: url(images/analysis/ArrowLeft${(props) =>
    props.$isEnd}.svg);
...
const ArrowRightImage = styled.div<{ $isEnd: string }>`
  background-image: url(images/analysis/ArrowRight${(props) =>
    props.$isEnd}.svg);
  background-size: cover;
  width: 16px;
  height: 16px;
...
function AnalysisPagenation(props: propsType) {
  ...
  return (
    <Pagenation>
      <ArrowLeftImage
        $isEnd={getIsPageStart(props.nowPageNum)}
        onClick={props.handleslidePrev}
      />
      <PagenationText>1 - {props.maxPageNum}</PagenationText>
      <ArrowRightImage
        $isEnd={getIsPageEnd(props.nowPageNum, props.maxPageNum - 1)}
        onClick={props.handleslideNext}
      />
    </Pagenation>
  );
}
